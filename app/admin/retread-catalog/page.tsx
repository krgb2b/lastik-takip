...
function AdminRetreadCatalogPageContent() {
  ...
  async function saveBrand() {
    ...
    if (editingBrandId) {
      ...
      setBrands((prev) =>
        sortByName(
          prev.map((brand) =>
            brand.id === editingBrandId ? { ...brand, name: nextName } : brand
          )
        )
      );
  ...
  function exportPatternsToExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "KaplamaDesenleri");
    XLSX.writeFile(wb, "kaplama-desenleri.xlsx");
  }
  ...
  async function importPatternsFromExcel(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (inserts.length > 0) {
        const { error: insertError } = await supabase.from("tread_patterns").insert(inserts);
        if (insertError) {
      ...
      const parts = [
        `${inserts.length} eklendi`,
        `${updates.length} güncellendi`,
      ];
      if (skipped.length > 0) {
